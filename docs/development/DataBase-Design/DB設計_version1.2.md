...
| No | テーブル論理名 | テーブル名 | 概要 |
| :--- | :--- | :--- | :--- |
| 1 | ユーザー情報テーブル | users | 顧客（ユーザー）のID、認証情報、残高、ポイントなどを管理する |
| 2 | 管理者情報テーブル | admins | システム管理者のログイン情報や名前を管理する |
| 3 | 商品情報テーブル | products | 提供するメニューの商品名、価格、画像、カテゴリなどを管理する |
| 4 | 注文情報テーブル | orders | 来店ごとの伝票情報、人数、合計金額などのヘッダー情報を管理する |
| 5 | 注文明細テーブル | order_items | 伝票ごとの具体的な注文商品、数量、注文時の価格などを管理する |
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | admin_name | VARCHAR(32) | YES | | 管理者名 (固定値) |
...
| 7 | update_date | DATETIME | YES | | 更新日時 |
| 8 | single_adult | INT | NO | | １人のお客様が注文した数 |
| 9 | two_adults | INT | NO | | 2人のお客様が注文した数 |
| 10 | family_group | INT | NO | | ファミリー（大人1人から2人＋子供1から7人）のお客様が注文した数 |
| 11 | adult_group | INT | NO | | 大人グループ（大人3人から8人）のお客様が注文した数 |
| 12 | group | INT | NO | | グループ（大人3人から7人＋子供1人から5人）のお客様が注文した数 |

<br>



### **テーブル名：Orders (伝票・決済情報)**

| No | カラム名 | データ型 | Not Null | Key | 和名（備考） |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | order_id | UUID | YES | PRIMARY | 伝票ID |
| 2 | user_id | UUID | YES | FOREIGN | ユーザーID |
| 3 | table_number | CHAR(4) | YES | | テーブル番号 (4桁の数字（0000から9999）)|
| 4 | adult_count | INT | YES | | 大人人数 (1人から8人) |
| 5 | child_count | INT | YES | | 子供人数 (0人から7人) |
| 6 | is_payment_completed | BOOL | YES | | 決済完了 |
| 7 | visit_date | DATETIME | YES | | 来店日時 |
| 8 | use_point | INT | NO | | 利用ポイント (0ポイントから合計金額) |
| 9 | earn_point | INT | NO | | 付与ポイント (決済金額の1%を付与し、端数は切り捨て) |
| 10 | payment_date | DATETIME | NO | | 決済日時  |

<br>


### **テーブル名：OrderItems (注文商品情報)**

| No | カラム名 | データ型 | Not Null | Key | 和名（備考） |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | order_item_id | UUID | YES | PRIMARY | 伝票項目ID |
| 2 | order_id | UUID | YES | FOREIGN | 伝票ID  |
| 3 | product_id | VARCHAR(32) | YES | FOREIGN | 商品ID |
| 4 | quantity | INT | YES | | 個数 (1個から10個) |
| 5 | price | INT | YES | | 価格 |
| 6 | add_date | DATETIME | YES | | 追加日時 |
| 7 | order_date | DATETIME | NO | | 注文確定日時 |
...
| 7 | login_attempt | INT | YES | | 試行回数（ログイン・決済失敗回数：3回失敗でユーザーロックアウト） |
